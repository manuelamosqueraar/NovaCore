...
const addProductToCart = (source) => {
    ...
    const image = source.dataset.image || panel?.dataset.image || imgEl?.src || resolveCartItemImage({ name, slug: source.dataset.slug || panel?.dataset.slug || '' });
    const slug = source.dataset.slug || panel?.dataset.slug || '';
